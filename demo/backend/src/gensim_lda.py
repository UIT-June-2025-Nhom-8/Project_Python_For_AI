...
Using PreProcessor methods (except normalize_token)
Replace with Lemmatization for LDA
"""
...
class GensimLDA:
    ...
    Using PreProcessor for cleaning but replace stemming with lemmatization
    """

    def __init__(self, random_state=42):
        self.random_state = random_state
        self.preprocessor = PreProcessor()  # Use existing PreProcessor
    ...
    def preprocess_for_lda(self, text):
        """
        Preprocessing pipeline for LDA using PreProcessor methods
        ...
        lemmatized_tokens = [
            self.lemmatizer.lemmatize(token.lower()) for token in tokens_no_stop
        ]
    ...
    def prepare_texts_for_lda(self, df, text_column="lda_input", sample_size=None):
        ...
        # Get the pre-processed tokens directly
        processed_texts = []
        for doc_tokens in df_sample[text_column]:  # <-- Iterate through token lists
            if (
                doc_tokens
            ):  # Only keep documents with tokens (after initial preprocessing)
                processed_texts.append(doc_tokens)

        print(f"Documents after preprocessing: {len(processed_texts)}")
        print(
            f"Average tokens per document: {np.mean([len(doc) for doc in processed_texts]):.2f}"
        )
    ...
    def find_optimal_topics(self, min_topics=10, max_topics=20):
        ...
        for i, num_topics in enumerate(range(min_topics, max_topics + 1)):
            print(
                f"| {num_topics:<16} | {coherence_scores[i]:<15.4f} | {perplexity_scores[i]:<16.4f} |"
            )
        ...
        # Plot results
        self.plot_metrics(
            range(min_topics, max_topics + 1), coherence_scores, perplexity_scores
        )
        ...
        return (
            optimal_topics,
            coherence_scores[optimal_idx],
            perplexity_scores[optimal_idx],
        )
    ...
    def plot_metrics(self, topic_range, coherence_scores, perplexity_scores):
        ...
        optimal_idx = np.argmax(coherence_scores)
        ax1.axvline(
            x=list(topic_range)[optimal_idx],
            color="r",
            linestyle="--",
    ...
    def create_pyldavis(self, save_html=True):
        ...
        vis = gensimvis.prepare(
            self.lda_model, self.corpus, self.dictionary, sort_topics=True
        )
    ...
    def load_model_components(self, path="models/gensim_lda_model"):
        ...
        try:
            self.dictionary = corpora.Dictionary.load(
                os.path.join(path, "dictionary.gensim")
            )
...
def run_lda_analysis(train_df, sample_size=50000, find_optimal=False, fixed_topics=15):
    ...
    processed_texts = lda.prepare_texts_for_lda(
        train_df, text_column="lda_input", sample_size=sample_size
    )
