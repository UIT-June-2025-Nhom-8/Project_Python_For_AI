...
class LGBMAnalyzer:
    """
    Lớp phân tích cảm xúc sử dụng LightGBM Classifier
    """
    
    def __init__(self):
        """Khởi tạo các thành phần của LGBMAnalyzer"""
        self.model = None
    ...
    def prepare_data(self, df, text_column, test_size=0.2, random_state=42):
        ...
        """
        print("Chuẩn bị dữ liệu cho LightGBM...")
    ...
    def initialize_model(self, **kwargs):
        """
        Khởi tạo LGBMClassifier với các tham số tùy chỉnh
        
        Args:
            **kwargs: Các tham số cho LGBMClassifier
        """
        default_params = {
            'num_leaves': 31,
            'learning_rate': 0.05,
        ...
        print(f"Khởi tạo LGBMClassifier với tham số: {default_params}")
        
    def train_model(self):
        """
        Huấn luyện model LGBMClassifier
        ...
        print("Bắt đầu huấn luyện LGBMClassifier...")
    ...
    def save_model(self, model_path='models/lightgbm_model.pkl'):
        """
    ...
    def load_model(self, model_path='models/lightgbm_model.pkl'):
        """
        Tải model đã lưu
    ...
    def get_model_info(self):
        ...
        info = {
            ...
            "bagging_fraction": self.model.bagging_fraction,
    ...
    def train_with_early_stopping(self, eval_metric='logloss', early_stopping_rounds=50):
        """
        Huấn luyện model với early stopping
        
        Args:
            eval_metric (str): Metric để đánh giá
            early_stopping_rounds (int): Số rounds để dừng sớm
        ...
        print("Bắt đầu huấn luyện LGBMClassifier với early stopping...")
        
        # Tạo validation set từ training data
        ...
        # Lưu kết quả
        self.results = {
            'model_name': 'LGBMClassifier (with Early Stopping)',
            'train_accuracy': train_acc,
            'test_accuracy': test_acc,
            'classification_report': report,
            'confusion_matrix': cm,
        ...
        print(f"Huấn luyện hoàn thành!")
        print(f"Best iteration: {self.results['best_iteration']}")
        print(f"Training Accuracy: {train_acc:.4f}")
        print(f"Test Accuracy: {test_acc:.4f}")
