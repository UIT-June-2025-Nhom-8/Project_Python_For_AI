...
def run_lda_experiments(
    ...
    """
    Run LDA for multiple n_topics values and return comparison DataFrame.
    Calculate train/test Perplexity, Log-likelihood, Log-perplexity (per-word)
    and Coherence (c_v if gensim is available).
    """
...
def plot_coherence_and_perplexity(
    ...
    *,
    ...
    if have_coh:
        ...
        ax1.axvline(
            best_coh_x, linestyle="--", color="red", label=f"Optimal: {best_coh_x}"
        )
        ...
        ax2.axvline(
            best_ppl_x, linestyle="--", color="red", label=f"Optimal: {best_ppl_x}"
        )
