...
class ModelTrainer:
    ...
    def prepare_data_for_models(self, train_df, test_df):
        ...
        print(
            f"Sentiment distribution: {combined_df['sentiment'].value_counts().to_dict()}"
        )

        return combined_df, text_column

    def train_logistic_regression(
        self, df, text_column, optimize_hyperparameters=False
    ):
        """
        ...
        try:
            # Khởi tạo analyzer
            lr_analyzer = LogisticRegressionAnalyzer(
            ...
            print(
                f"Logistic Regression training completed in {training_time:.2f} seconds"
            )
    ...
    def train_gradient_boosting(self, df, text_column):
        ...
        try:
            ...
            print(
                f"Gradient Boosting training completed in {training_time:.2f} seconds"
            )
    ...
    def train_all_models(self, train_df, test_df, optimize_hyperparameters=False):
        ...
        models_to_train = [
            (
                "Logistic_Regression",
                lambda: self.train_logistic_regression(
                    combined_df, text_column, optimize_hyperparameters
                ),
            ),
            (
                "Random_Forest",
                lambda: self.train_random_forest(
                    combined_df, text_column, optimize_hyperparameters
                ),
            ),
            (
                "Gradient_Boosting",
                lambda: self.train_gradient_boosting(combined_df, text_column),
            ),
        ]
        ...
        # Tạo summary
        pipeline_summary = self.create_pipeline_summary(
            all_results, total_pipeline_time
        )
    ...
    def create_pipeline_summary(self, all_results, total_time):
        ...
        if accuracies:
            ...
            summary["comparison"] = {
                ...
                "all_training_times": {
                    k: round(v, 2) for k, v in training_times.items()
                },
            }
    ...
    def print_summary(self, results):
        ...
        print(
            f"Total Pipeline Time: {summary.get('total_pipeline_time_seconds', 0)} seconds"
        )
    ...
    def run_training_pipeline(
        self, train_df, test_df, optimize_hyperparameters=False, save_results=True
    ):
        """
