...
class ModelTrainer:
    ...
    def __init__(self, output_dir="reports"):
        ...
        # Define available models for training
    ...
    def run_training_pipeline_with_configs(
        self, train_df, test_df, model_configs, save_results=True
    ):
        """
        Run training pipeline with configs from JSON file

        Args:
            train_df (pd.DataFrame): Training data with 'sentiment' column
            test_df (pd.DataFrame): Test data with 'sentiment' column
            model_configs (dict): Configs loaded from JSON
        ...
        print(
            f"Models trained: {len([r for r in all_results.values() if 'error' not in r])}/{len(all_results)}"
        )
